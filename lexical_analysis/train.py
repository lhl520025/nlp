...
parser.add_argument("--model_save_dir", type=str, default="./output", help="The model will be saved in this path.")
parser.add_argument("--epochs", type=int, default=20, help="Corpus iteration num.")
...
def train(args):
    ...
    train_ds, test_ds = load_dataset(datafiles=(os.path.join(
        args.data_dir, 'train_ceshi.tsv'), os.path.join(args.data_dir, 'test.tsv')))
        ...
        for step, batch in enumerate(train_loader):
            ...
            if global_step % args.logging_steps == 0:
                logger.info(
                    ...
                    % (global_step, last_step, avg_loss, train_reader_cost /args.logging_steps, (train_reader_cost + train_run_cost)/ args.logging_steps, total_samples / args.logging_steps,
            ...
            if global_step % args.save_steps == 0 or global_step == last_step:
                if rank == 0:
                    paddle.save(model.state_dict(),os.path.join(args.model_save_dir,"model_%d.pdparams" % global_step))
...
if __name__ == "__main__":
    ...
    #lac = LAC(model_path='E:\\npl_workspace\PaddleNLP\\examples\\lexical_analysis\\output\\best_model.pdparams')
    # lac = LAC(mode='lac')
    # text = u"LAC是个优秀的分词工具"
    # print(lac.run(text))
    # ner = Taskflow("ner")
    ...
    #
    # print(wordtag("LAC是个优秀的分词工具"))
    # print(lac("LAC是个优秀的分词工具"))
