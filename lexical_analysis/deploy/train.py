...
import json
# C:\Users\Administrator.BF-20210813GIMU\PycharmProjects\NLPProject1\infer_model\home\aistudio\infer_model\static_graph_params.pdmodel
# yapf: disable
parser = argparse.ArgumentParser(__doc__)

parser.add_argument("--model_file", type=str, default='E:\\npl_workspace\\PaddleNLP\\examples\\lexical_analysis\\infer_model\\static_graph_params.pdmodel', help="The path to model info in static graph.")
parser.add_argument("--params_file", type=str, default='E:\\npl_workspace\\PaddleNLP\\examples\\lexical_analysis\\infer_model\\static_graph_params.pdiparams', help="The path to parameters in static graph.")
parser.add_argument("--data_dir", type=str, default="./data/lexical_analysis_dataset_tiny", help="The folder where the dataset is located.")
...
word_vocab = load_vocab(
    'E:\\npl_workspace\\PaddleNLP\\examples\\lexical_analysis\\data\\lexical_analysis_dataset_tiny\\word.dic')
label_vocab = load_vocab(
    'E:\\npl_workspace\\PaddleNLP\\examples\\lexical_analysis\\data\\lexical_analysis_dataset_tiny\\tag.dic')
normlize_vocab = load_vocab(
    'E:\\npl_workspace\\PaddleNLP\\examples\\lexical_analysis\\data\\lexical_analysis_dataset_tiny\\q2b.dic')
...
def get_result(txts):
    ...
    list = []
    # print(results[0])
    for text,result in zip(results[0][0],results[0][1]):
        if result != 'O':
            if (result+text) not in list:
                list.append(result+text)
                data_list.append({'text':text,'ner':result})
    return data_list
...
def api_use():
    ...
    if text_json['text'].strip()!='':
        data_list=get_result(text_json['text'])
...
if __name__ == "__main__":
    app.run(host="192.168.200.161",port=3797, debug=True)